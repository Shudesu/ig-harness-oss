...
Instagram向けオープンソース DM自動化 / マーケティングオートメーション。
...
| 機能 | Instagram Harness | ManyChat | iステップ |
|------|-------------------|----------|-----------|
| 月額料金 | **$0〜** | $15〜 | ¥14,800〜 |
| エンゲージメントゲート | ✅ | ✅ | ✅ |
| フォロー check ループ | ✅ | ✅ (Pro) | ✅ |
...
@ig-harness/mcp-server → Workers API → D1
         ...
         ┌────────────── UUID 共有 webhook ──────────────┐
         ▼                                                ▼
  Instagram Harness ←──── 共有シークレット ────→ LINE Harness
...
Instagram Harness の目玉機能。投稿 / リールに特定キーワードでコメントしたユーザーをトリガーに、フォロー状態を確認 → 未フォローなら "フォローしてね" DM → フォロー後に特典 DM を自動配布します。
